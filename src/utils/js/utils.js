...
function lrcReg(data) {
    ...
    for (let i in lrcArr) {
        ...
        let tempTime = _ele.match(timeReg).toString();
...
function limitLenth(_obj) {
    ...
    for (let i in _obj) {
        if (_obj[i].length > 30) {
            ...
            let newStrAft = Array.prototype.slice.call(_obj[i], spaceIndex + 1).join('');
            let newStrPre = Array.prototype.slice.call(_obj[i], 0, spaceIndex).join('');
